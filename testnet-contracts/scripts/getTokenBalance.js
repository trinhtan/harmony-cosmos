module.exports = async () => {
  ...
  try {
    ...
    const contract = truffleContract(require('../build/contracts/BridgeToken.json'));
    ...
    return console.log(
      `Balance of ${name} for ${account} is ${balanceEth.toString(
        10
      )} ${symbol} (${balanceWei} ${symbol} with ${decimals} decimals)`
    );
  } catch (error) {
    console.error({ error });
  }
